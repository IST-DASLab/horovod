...
MPI_Quantized_CUDAAllreduce::MPI_Quantized_CUDAAllreduce(
  ...
  quantized_buffer_size =
      ceil(1.0 * chunk_size * global_state->quantization_bits / sizeof(float));
...
Status MPI_Quantized_CUDAAllreduce::Init(
  ...
  int64_t buffer_size = chunk_size + 2 * maxmin_buffers_size +
  ...
  quantized_gradients_recv = quantized_gradients_send +
                             quantized_buffer_size * (global_state_->size - 1);
  cuda_states =
      (curandState*)(quantized_gradients_recv +
                     quantized_buffer_size * (global_state_->size - 1));
...
MPI_Quantized_CUDAAllreduce::Execute(std::vector<TensorTableEntry>& entries,
  ...
  for (int division = 0; division < num_divisions; division++) {
    ...
    count = 0;
    for (int node_rank = 0; node_rank < num_nodes; node_rank++) {
      if (node_rank == rank) { // for each node(chunk)
        continue;
      }
    ...
    MPI_Waitall((int)request_reduce.size(), &request_reduce[0],
                MPI_STATUSES_IGNORE);
    auto mpi_end = now();
    ...
    for (int node_rank = 0; node_rank < num_nodes; node_rank++) {
      ...
      MPI_Irecv(maxandmin_send + count * maxmin_size, nb * 2,
                mpi_context_->GetMPIDataType(first_entry.tensor), node_rank, 0,
                MPI_COMM_WORLD, &request_gather.back());
    ...
    MPI_Waitall((int)request_gather.size(), &request_gather[0],
                MPI_STATUSES_IGNORE);
    mpi_end = now();
    global_state_->communication_time += mpi_end - mpi_start;
