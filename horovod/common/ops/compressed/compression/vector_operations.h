...
public:
  Summator() = default;
  // @inplace stands for using output tensor from entry as input or original one.
  // entry.output = x + (entry.output|entry.tensor)
  // if inplace set true use output.
  void Add(float* x, std::vector<TensorTableEntry>& entries,
           int64_t fusion_offset, int64_t global_offset, int64_t num_elems,
           bool inplace);
  virtual void Add(float* x, float* y, float* sum, int64_t num_elems, int device) = 0;
};
...
public:
  ...
  void Add(float* x, float* y, float* sum, int64_t num_elems, int device);
