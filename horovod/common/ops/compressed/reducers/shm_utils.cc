...
static int shmSetup(const char* shmname, const int shmsize, int* fd, void** ptr,
                    int create) {
...
static int shmOpen(const char* shmname, const int shmsize, void** shmPtr,
...
void shmComm::sendInit(shmBuffer* buffer, int peer_rank, size_t shm_size) {
...
void shmComm::recvInit(shmBuffer* buffer, int peer_rank, size_t shm_size) {
  char shmName[MAX_SHM_NAME_LEN];
  buffer->shmSize = shm_size;
...
void shmComm::Send(void* buf, size_t buf_size, int peer_rank,
  ...
  CUDA_CHECK(cudaMemcpyAsync((void*)(((char*)shm_buf.devHostMem) + offset), buf,
                             buf_size, cudaMemcpyDeviceToDevice, stream));
  CUDA_CHECK(cudaEventRecord(eventSync.event, stream));
  MPI_CHECK(MPI_Isend((void*)&eventSync.dummy, 1, MPI_UNSIGNED_CHAR, peer_rank,
                      0, comm_, &eventSync.request));
}
...
void shmComm::post_sendBuf(int peer_rank, cudaStream_t stream) {
  ...
  CUDA_CHECK(cudaEventRecord(eventSync.event, stream));
  MPI_CHECK(MPI_Isend((void*)&eventSync.dummy, 1, MPI_UNSIGNED_CHAR, peer_rank, 0,
                      comm_, &eventSync.request));
}

int shmComm::RecvBufAsync(void** buf, int peer_rank, cudaStream_t stream,
                          size_t offset) {
  auto& recv_resource = recv_resources[peer_rank];
  ...
  *buf = nullptr;
  if (eventSync.request == MPI_REQUEST_NULL) {
    MPI_CHECK(MPI_Irecv(&eventSync.dummy, 1, MPI_UNSIGNED_CHAR, peer_rank, 0, comm_,
                        &eventSync.request));
  }
  int flag;
  ...
  *buf = (void*)(((char*)shm_buf.devHostMem) + offset);
  eventSync.request = MPI_REQUEST_NULL;
  return 1;
}

void shmComm::RecvBuf(void** buf, int peer_rank, cudaStream_t stream,
                      size_t offset) {
  auto& recv_resource = recv_resources[peer_rank];
  auto& shm_buf = recv_resource.first;
  auto& eventSync = recv_resource.second;
  *buf = nullptr;
  MPI_CHECK(MPI_Recv(&eventSync.dummy, 1, MPI_UNSIGNED_CHAR, peer_rank, 0,
                     comm_, MPI_STATUSES_IGNORE));
  CUDA_CHECK(cudaStreamWaitEvent(stream, eventSync.event, 0));
  *buf = ((char*)shm_buf.devHostMem) + offset;
}
...
void shmComm::freeBuffer(shmBuffer* buffer) {
  TRIV_CHECK(shmClose(buffer->hostMem, buffer->devHostMem, buffer->shmSize));
}
