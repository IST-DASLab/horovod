...
Status MPI_GPUAllreduce_Ring::AllreduceDivision(
  ...
  for (int i = 0; i < world_size - 1; i++) {
    ...
    MPI_Irecv(gradients_recv_, recv_size, MPI_UNSIGNED_CHAR, recv_from, 0, comm,
              &recv_req);
    send_size = round_to(compressor_->Compress(gradients_send_, entries,
    ...
    summator_->Add((float*)decompress_buffer_, entries, buf_recv_idx, global_offset,
                  segment_size(recv_segment_idx), false);
  }
  ...
  send_size =
      round_to(compressor_->Compress(send_buf,
                                     entries, buf_send_idx, global_offset,
                                     segment_size(send_segment_idx), false),
               ALIGNMENT_UNIT);
  compressor_->Decompress(send_buf, entries,
                          buf_send_idx, global_offset,
                          segment_size(send_segment_idx));
